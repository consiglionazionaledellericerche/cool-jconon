...
public class CommonRepository {
    ...
    private List<Map<String, Serializable>> managerCalls(String userId, Session session) {
        try {
            List<Map<String, Serializable>> calls = new ArrayList<Map<String, Serializable>>();
            Criteria criteriaCommissions = CriteriaFactory.createCriteria(JCONONFolderType.JCONON_CALL.queryName());
            criteriaCommissions.addColumn(PropertyIds.OBJECT_ID);
            criteriaCommissions.addColumn(PropertyIds.OBJECT_TYPE_ID);
            criteriaCommissions.addColumn(JCONONPropertyIds.CALL_CODICE.value());
            criteriaCommissions.addColumn(JCONONPropertyIds.CALL_PUBBLICATO.value());
            criteriaCommissions.addColumn(JCONONPropertyIds.CALL_DATA_INIZIO_INVIO_DOMANDE.value());
            criteriaCommissions.add(Restrictions.eq(PropertyIds.CREATED_BY, userId));
            criteriaCommissions.addOrder(Order.descending(PropertyIds.CREATION_DATE));
            ItemIterable<QueryResult> iterableCommission = criteriaCommissions.executeQuery(session, false, session.getDefaultContext());
            for (QueryResult queryResult : iterableCommission.getPage(10)) {
                Boolean publish = queryResult.<Boolean>getPropertyValueById(JCONONPropertyIds.CALL_PUBBLICATO.value());
                String callType = queryResult.<String>getPropertyValueById(PropertyIds.OBJECT_TYPE_ID);

                Calendar dataInizioInvioDomande = queryResult.<Calendar>getPropertyValueById(JCONONPropertyIds.CALL_DATA_INIZIO_INVIO_DOMANDE.value());
                calls.add(
                        Stream.of(
                                new AbstractMap.SimpleEntry<>("id",
                                        String.format(
                                                "manage-call?call-type=%s&cmis:objectId=%s",
                                                callType,
                                                queryResult.<String>getPropertyValueById(PropertyIds.OBJECT_ID)
                                        )
                                ),
                                new AbstractMap.SimpleEntry<>("absolute", Boolean.TRUE),
                                new AbstractMap.SimpleEntry<>("display", Boolean.TRUE),
                                new AbstractMap.SimpleEntry<>("disabled", Boolean.FALSE),
                                new AbstractMap.SimpleEntry<>(PropertyIds.OBJECT_TYPE_ID, callType),
                                new AbstractMap.SimpleEntry<>("title",
                                        String.format(
                                                "<h4 class=\"list-group-item-heading\">%s</h4><p class=\"list-group-item-text\"><small>%s del %s, %s</small><p>",
                                                queryResult.<String>getPropertyValueById(JCONONPropertyIds.CALL_CODICE.value()),
                                                i18NService.getLabel(callType, Locale.ITALY),
                                                Optional.ofNullable(dataInizioInvioDomande).map(Calendar::getTime).map(CallService.DATEFORMAT::format).orElse(""),
                                                publish ? "<span class=\"label label-success\">pubblicato</span>" : "<span class=\"label label-warning\">non ancora pubblicato</span>"
                                        )
                                ),
                                new AbstractMap.SimpleEntry<>("publish", publish)
                        ).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue))
                );
            }
            return calls;
	...
	@Cacheable(value = "managers-calls", key = "#userId")
	public List<Map<String, Serializable>> getManagerCalls(String userId, Session session) {
		return managerCalls(userId, session);
	}
