...
public class ExportApplications {
	...
	public Response exportApplications(@Context HttpServletRequest req,
			...
			@QueryParam("deleteFinalFolder") boolean deleteFinalFolder) {
		  
		Map<String, Object> model = new HashMap<String, Object>();
		ResponseBuilder rb;
		try {
		  String noderefFinalZip = exportApplicationsService.exportApplications(cmisService.getCurrentCMISSession(req.getSession(false)), cmisService.getCurrentBindingSession(req), store_type + "://" + store_id + "/" + id, deleteFinalFolder);
			model.put("url", SEARCH_CONTENT + noderefFinalZip
