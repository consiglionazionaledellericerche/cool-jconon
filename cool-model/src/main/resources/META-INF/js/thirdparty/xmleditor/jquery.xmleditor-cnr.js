...
$.widget( "xml.xmlEditor", {
	options: {
		...
		ajaxOptions : {
			xmlUploadPath: null,
			xmlUploadFunction: null,
			xmlUploadFileName: null,
		...
		promptOnDelete: [],
		removeTestFunc: function() {return true;}
	},
	...
	submitXML: function() {
	  ...
	  if(xmlString) {
	    if(this.options.ajaxOptions.xmlUploadFunction) {
	      this.options.ajaxOptions.xmlUploadFunction(xmlString);
	    } else {
	      this.defaultSubmit(xmlString);
	    }
	  }
	},
	
	defaultSubmit: function(xmlString) {	  
...
//Modificato
	//setta type name ed il property name con il prefix del model nella creazione di nuovi Type e properties
	if(parentElement.guiElement.attr('class').indexOf("dd_typeInstance") >= 0 || parentElement.guiElement.attr('class').indexOf("dd_propertyInstance") >= 0) {
		attribute.attributeInput.val($("[class*='dd_namespacesInstance'] :input").val() + ":");
	}
//Fine Modifica
...
function MenuBar(editor) {
    ...
    action : function(event) {
      self.editor.options.ajaxOptions.xmlRetrievalPath = '/cool-jconon/search/content?nodeRef=workspace://SpacesStore/39d9063d-5b1f-4f88-8750-7b9f5337b828;1.10';
    ...
    items: [
            {
              ...
              enabled : true,
              action : function(event) {
                $('.xmleditor').find('.top_level_element_group').show();
              }
            }, 
            {
              label : 'Types',
              enabled : true,
              action : function(event) {
                $('.xmleditor').find('.top_level_element_group').hide().find(":contains('dd:types')").closest('.top_level_element_group').show();
//                $('.xmleditor').find('.top_level_element_group').find(":contains('dd:types')").closest('.top_level_element_group').show();
              }
            }, {
              label : 'Aspects',
              enabled : true,
              action : function(event) {
                $('.xmleditor').find('.top_level_element_group').hide().find(":contains('dd:aspects')").closest('.top_level_element_group').show();
//                $('.xmleditor').find('.top_level_element_group').find(":contains('dd:aspects')").closest('.top_level_element_group').show();
              }
            }, {
              label : 'Constraints',
              enabled : true,
              action : function(event) {
                $('.xmleditor').find('.top_level_element_group').hide().find(":contains('dd:constraints')").closest('.top_level_element_group').show();
//                $('.xmleditor').find('.top_level_element_group').find(":contains('dd:constraints')").closest('.top_level_element_group').show();
              }
            }
    ]
  }, 
...
XMLElement.prototype.remove = function() {
  ...
  if( this.editor.options.removeTestFunc(this) ) {
    this.removeExecute();
  } 
};

XMLElement.prototype.removeExecute = function() {
